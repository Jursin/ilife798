...
actual suspend fun payWithAlipay(orderInfo: String): AlipayPayResult = withContext(Dispatchers.IO) {
    ...
    try {
        ...
        when (status) {
            "9000" -> AlipayPayResult(true, status, "充值成功")
            "8000", "6004" -> AlipayPayResult(false, status, "支付结果确认中，请稍后刷新余额")
            "6001" -> AlipayPayResult(false, status, "已取消支付")
            else -> AlipayPayResult(false, status, if (memo.isBlank()) "支付失败" else "支付失败：$memo")
        }
    } catch (e: Exception) {
